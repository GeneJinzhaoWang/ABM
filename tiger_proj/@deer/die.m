...
%death function for class Deer
% agt: deer object
% cn: current number of agent


%Deer dies if its food level reaches zero, or its water level reaches zero 
% or they are older than max_age
    ...
    if cfood<=0 || cwater <=0 || age>PARAM.D_MAXAGE
        ITER_STATS.died_deer(NO_IT+1)=ITER_STATS.died_deer(NO_IT+1)+1;      %update statistics
