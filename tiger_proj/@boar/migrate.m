...
%MIGRATE Summary For BOAR
%   Detailed explanation goes here
    global ENV_DATA PARAM
    target= 0;
    boar_pos = agt.position;
    spd=agt.speed; 
    to_destiny = false;
%   Decision change from 0 to 2.
% decision==0 means the boar will move randomly.
% decision==1 means the boar is in thirst and will move towards the nearest pool.
% decision==2 means the boar is in hunger and will move towards the nearest food.
    switch decision
%       Move randomly
        case 0
...
%       Move towards pool
         case 1
            ...
            dist2pools=sqrt((pool_centers(:,1)-boar_pos(:,1)).^2+(pool_centers(:,2)-boar_pos(:,2)).^2);  %calculate distance to all prey
...
%         Move towards food    
        case 2
            % See if there's any food nearby
